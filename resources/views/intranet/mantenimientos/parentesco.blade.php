...
@section('content')
    <div id="pcont" class="container-fluid">
        <div class="page-head">
            <h2 style="display:inline-block;">Parentesco</h2>
                    ...
                    <div class="tab-container">
                        ...
                        <div class="tab-content">
                            ...
                            <div id="tp2" class="tab-pane cont">
                                <div class="container">
                                    <form id="frmParentesco" method="post" data-parsley-validate="" data-parsley-excluded="[disabled=disabled]" novalidate="">
                                        <input type="hidden" id="hddCodigo" value="">
                                        <div class="row">
                                            <label for="txtNombre" class="col-md-1 control-label">Nombre</label>
                                            <div class="col-md-5">
                                                <input id="txtNombre" type="text" placeholder="Ejem. EDUCACION,DOCENTE,FISICA,HISTORIA" class="form-control"
...
@section('scripts')
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#frmParentesco").parsley();
            listar();
        });

        function guardar(){
            var accion = $("#hddCodigo").val() == "" ? true : false;
            if($("#frmParentesco").parsley().validate()){
                if (accion)
                    registrar()
                else
                    modificar()
        ...
        function modificar() {
            if (confirm("¿Deseas continuar la modificación?")) {
                var info = obtenerDatos();
                $.ajax({
                    url: "/intranet/mantenimientos/parentesco/" + $("#hddCodigo").val(),
                    type: "PUT",
                    data: info,
                    beforeSend: function () {
                        $("#loading").show();
        ...
        function eliminar(id) {
            if (confirm("¿Deseas eliminar el elemento?")) {
                ...
                $.ajax({
                    url: "/intranet/mantenimientos/parentesco/" + id,
                    type: "DELETE",
                    data: info,
                    beforeSend: function () {
                        $("#loading").show();
